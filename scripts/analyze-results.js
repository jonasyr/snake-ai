...
function loadResults(filePath) {
  try {
    ...
    } else if (results.configurations && Array.isArray(results.configurations)) {
...
function analyzeConfigurations(results) {
  ...
  console.log(`✅ ${successful.length} configurations with ≥95% completion rate`);
  ...
  const analysis = {
    ...
    improvement: ((Math.max(...moves) - Math.min(...moves)) / Math.max(...moves) * 100).toFixed(1)
  };
...
function findParameterInsights(results) {
  ...
  for (const param of parameters) {
    const values = results.map(r => r.config[param]).filter(v => v !== undefined);
    ...
    for (const item of top3) {
      console.log(`    ${item.value}: ${item.avgMoves.toFixed(0)} moves (${item.count} configs)`);
    }
  }
...
function generateRecommendations(successful, insights) {
  ...
  for (let i = 0; i < top5.length; i += 1) {
    const config = top5[i];
    console.log(`  ${i + 1}. ${config.averageMoves.toFixed(0)} moves - Safety: ${config.config.safetyBuffer}, Lock: ${config.config.lateGameLock}, Window: ${config.config.minShortcutWindow}`);
  }
  ...
  const conservativeIndex = successful.findIndex(s => s.config.safetyBuffer > bestConfig.safetyBuffer);
  if (conservativeIndex > 0) {
    ...
    console.log(`     minShortcutWindow: ${conservative.config.minShortcutWindow}`);
    console.log(`     Expected moves: ~${conservative.averageMoves.toFixed(0)}`);
  }
  
  // Aggressive option
  const aggressiveIndex = successful.findIndex(s => s.config.safetyBuffer < bestConfig.safetyBuffer);
  if (aggressiveIndex > 0) {
    ...
    console.log(`     minShortcutWindow: ${aggressive.config.minShortcutWindow}`);
...
async function main() {
  ...
  if (filtered.length === 0) {
    console.error(`❌ No configurations meet the ${(minSuccess * 100).toFixed(0)}% success criteria`);
    process.exit(1);
  }

  const { successful, analysis } = analyzeConfigurations(filtered);
