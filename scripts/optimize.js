...
class MemoryMonitor {
  ...
  log(progress = null) {
    ...
    if (progress && progress.completed > 0 && progress.total > 0) {
      ...
      const etaStr = etaMinutes > 0 ? `${etaMinutes}m ${etaSeconds % 60}s` : `${etaSeconds}s`;
  ...
  for (let i = 0; i < configurations.length; i += batchSize) {
    ...
    for (let j = 0; j < batch.length; j += 1) {
      ...
      try {
        ...
        console.log(`  ${progress} ✓ ${completion} completion, ${moves} avg moves (${performance})`);
        ...
        console.error(`  [${configIndex}/${totalConfigs}] ❌ Error: ${error.message}`);
      }
    }
...
function createResultsStreamer(outputFile) {
  ...
  return {
    ...
    close(metadata = {}) {
      ...
      fs.appendFileSync(outputFile, '\n  ],\n  "finalMetadata": ' + JSON.stringify(finalMetadata, null, 2) + '\n}');
    }
  };
}
...
async function adaptiveOptimization(options = {}) {
  ...
  console.log(`💻 System: ${sysInfo.cpuCores} cores, ${sysInfo.totalMemoryGB}GB RAM (using ${sysInfo.maxMemoryMB}MB)`);
  console.log(`🎯 Initial samples: ${initialSamples}, Refinement: ${refinementRounds} rounds × ${refinementSamples} samples`);
  ...
  try {
    ...
    for (let round = 0; round <= refinementRounds; round += 1) {
      ...
      const results = await processBatch(samples, { games, batchSize: 5 }, monitor);
      ...
      if (roundBest.length > 0) {
        ...
        console.log(`🏆 Found ${roundBest.length} successful configs this round`);
        console.log(`   Best: ${roundBest[0].averageMoves.toFixed(1)} moves (${(roundBest[0].completionRate * 100).toFixed(1)}%)`);
      } else {
      ...
      if (round < refinementRounds && roundBest.length > 0) {
        ...
        samples = generateLatinHypercubeSamples(currentRanges, refinementSamples);
      }
    ...
    if (finalBest.length > 0) {
      ...
      for (const config of finalBest) {
        console.log(`   ${config.averageMoves.toFixed(1)} moves (${(config.completionRate * 100).toFixed(1)}%) - ${JSON.stringify(config.config)}`);
      }
    }
...
function createRefinedRanges(successfulConfigs, originalRanges) {
  ...
  for (const [key, originalRange] of Object.entries(originalRanges)) {
    const values = successfulConfigs.map(c => c.config[key]).filter(v => v !== undefined);
